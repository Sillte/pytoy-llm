...
class TaskRequest[T: BaseModel | str](BaseModel, frozen=True):
    ...
    def validate_spec(
        cls,
        spec: FunctionInvocationSpec[T]
        | LLMInvocationSpec[T]
        | AgentInvocationSpec[T]
        | SelectedInvocationSpec[T]
        | TaskSpec[T],
    ) -> TaskSpec[T]:
        if isinstance(spec, TaskSpec):
            return spec
        else:
            return TaskSpec.from_single_spec("TaskSpecFromSinpleSpec", spec)
