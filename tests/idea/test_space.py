...
def test_space_preserves_custom_note_predicator_in_subspaces(tmp_path: Path) -> None:
    nested = tmp_path / "nested"
    nested.mkdir()
    (nested / "note.txt").write_text("note", encoding="utf-8")

    space = IdeaSpace(tmp_path, note_predicator=lambda path: path.suffix == ".txt")

    assert [note.path for note in space.get_subspaces()[0].get_notes()] == ["nested/note.txt"]
